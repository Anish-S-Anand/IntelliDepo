...
# ---------------------------------------------------------------------------
# Daily Throughput — Bags In / Out (7-day history)
# ---------------------------------------------------------------------------

class DailyThroughputEntry(BaseModel):
    day: str          # e.g. "Mon", "Tue"
    date: str         # e.g. "2026-05-12"
    bags_in: int      # bags received from counted inbound tracked objects
    bags_out: int     # bags dispatched from counted outbound tracked objects


@router.get("/throughput/daily", response_model=list[DailyThroughputEntry])
async def get_daily_throughput(
    db: AsyncSession = Depends(get_db),
):
    """
    Returns 7-day daily throughput from real tracked bag movements.
    Bags in/out are counted only when tracked bag objects cross the counting
    line with an inbound/outbound direction. Days with no events return 0.
    """
    from datetime import timedelta
    from app.depot.vision.tracking import TrackedObject

    today = datetime.now(timezone.utc).date()
    days = [(today - timedelta(days=i)) for i in range(6, -1, -1)]

    # Bags in - counted inbound bag tracks that crossed the line.
    bags_in_result = await db.execute(
        select(
            func.date(TrackedObject.created_at).label("day"),
            func.count(TrackedObject.id).label("total"),
        )
        .where(
            TrackedObject.class_label == "bag",
            TrackedObject.is_counted == True,
            TrackedObject.crossed_line == True,
            TrackedObject.direction == "inbound",
        )
        .group_by(func.date(TrackedObject.created_at))
    )
    bags_in_map = {str(row.day): int(row.total or 0) for row in bags_in_result.fetchall()}

    # Bags out - counted outbound bag tracks that crossed the line.
    bags_out_result = await db.execute(
        select(
            func.date(TrackedObject.created_at).label("day"),
            func.count(TrackedObject.id).label("total"),
        )
        .where(
            ...
            TrackedObject.is_counted == True,
            TrackedObject.crossed_line == True,
            TrackedObject.direction == "outbound",
        )
        .group_by(func.date(TrackedObject.created_at))
    )
    bags_out_map = {str(row.day): int(row.total or 0) for row in bags_out_result.fetchall()}

    result = []
    for d in days:
        ds = str(d)
        result.append(DailyThroughputEntry(
            day=d.strftime("%a"),
            date=ds,
            bags_in=bags_in_map.get(ds, 0),
            bags_out=bags_out_map.get(ds, 0),
        ))
    return result
