...
import { ensureTenantProfile, getTenantProfile, updateTenantProfile } from "@/services/macropulse";
...
export default function ConfigLivePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setProfile(await getTenantProfile(tenantId));
      } finally {
        setLoading(false);
      }
    };
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage({ text: "Unable to save configuration.", ok: false });
    } finally {
      setSaving(false);
    }
  };
